        ...
        <div className="flex gap-4 overflow-x-auto pb-4">
          {TRACKER_STATUSES.map((status) => (
            <div key={status} className="w-72 shrink-0">
              ...
              <div className="border border-t-0 border-hairline rounded-b-xl bg-sunken p-2 space-y-2 min-h-[120px]">
                {columns[status].length === 0 ? (
                  <p className="text-xs text-faint text-center py-4">Empty</p>
                ) : (
                  columns[status].map((job) => (
                    <div key={job.id} className="bg-surface rounded-lg border border-hairline p-3 space-y-2">
                      <Link href={`/jobs/${job.id}`} className="block">
                        <p className="text-sm font-medium text-primary truncate hover:text-accent-text">
                          {job.title}
                        </p>
                        <p className="text-xs text-tertiary truncate">{job.company}</p>
                      </Link>
                      {job.matchResults?.[0] && (
                        <MatchScoreBadge
