...
export default function AgentDiagnosticsPage() {
  ...
  return <div className="max-w-5xl mx-auto px-8 py-10 space-y-6">
    <header className="flex justify-between gap-4"><div><h1 className="text-2xl font-semibold">Agent Diagnostics</h1><p className="text-sm text-secondary">Honest runtime checks. Safe tests never submit.</p></div><button onClick={test} disabled={testing} className="rounded-lg bg-accent text-white px-4 py-2 text-sm disabled:opacity-40">{testing ? "Running safe tests..." : "Test Agent Safely"}</button></header>
    <section className="grid md:grid-cols-2 gap-3">{Object.entries(data?.checks ?? {}).map(([key, value]) => <div key={key} className="bg-surface border rounded-lg p-4 flex justify-between gap-3"><div><div className="font-medium">{LABELS[key] ?? key}</div>{value.detail && <p className="text-xs text-tertiary">{value.detail}</p>}</div><strong className={value.pass ? "text-verified" : "text-critical"}>{value.pass ? "PASS" : "FAIL"}</strong></div>)}</section>
    <section className="bg-surface border rounded-lg p-5 text-sm space-y-2">
      ...
      {data?.completeErrorStack && <details className="rounded border border-hairline p-3"><summary className="cursor-pointer font-medium">Show details</summary><pre className="mt-3 bg-sunken text-primary border border-hairline rounded p-3 overflow-auto text-xs whitespace-pre-wrap">{data.completeErrorStack}</pre></details>}
    ...
    <section className="bg-surface border rounded-lg p-5 text-sm space-y-1">
      <h2 className="font-semibold">Real local Ollama vision preflight</h2>
      {vision ? <>
        <p className={vision.pass ? "text-verified" : "text-critical"}>{vision.pass ? "PASS" : "FAIL"} - Ollama {vision.ollamaVersion ?? "unknown"} - {vision.model ?? "unknown model"}</p>
        ...
        <details className="rounded border border-hairline p-3"><summary className="cursor-pointer font-medium">Show preflight response details</summary><pre className="mt-3 bg-sunken text-primary border border-hairline rounded p-3 overflow-auto text-xs whitespace-pre-wrap">{JSON.stringify(vision.tests, null, 2)}</pre></details>
      </> : <p className="text-caution">No real screenshot preflight has been recorded.</p>}
    </section>
    <section className="bg-surface border rounded-lg p-5 text-sm space-y-1"><h2 className="font-semibold">Real public Greenhouse inspection (read-only)</h2>{greenhouse ? <><p className={greenhouse.pass ? "text-verified" : "text-critical"}>{greenhouse.pass ? "PASS" : "FAIL"} - {greenhouse.labelCount ?? 0} labels - inspected {greenhouse.inspectedAt ?? "unknown"}</p><p>Personal data entered: {String(greenhouse.enteredPersonalData)} - submitted: {String(greenhouse.submitted)}</p>{greenhouse.url && <a className="text-accent-text hover:underline" href={greenhouse.url} target="_blank" rel="noopener noreferrer">Inspected public page</a>}</> : <p className="text-caution">No real-page inspection report has been recorded.</p>}</section>
    <section className="bg-surface border rounded-lg p-5 text-sm space-y-1"><h2 className="font-semibold">Real public Lever navigation (read-only)</h2>{lever ? <><p className={lever.pass ? "text-verified" : "text-critical"}>{lever.pass ? "PASS" : "FAIL"} - {lever.fieldCount ?? 0} fields - inspected {lever.inspectedAt ?? "unknown"}</p><p>Form detected: {String(lever.formDetected)} - personal data entered: {String(lever.enteredPersonalData)} - files uploaded: {String(lever.uploadedFiles)} - submitted: {String(lever.submitted)}</p>{lever.finalUrl && <a className="text-accent-text hover:underline" href={lever.finalUrl} target="_blank" rel="noopener noreferrer">Inspected form URL</a>}</> : <p className="text-caution">No real Lever navigation report has been recorded.</p>}</section>
    <section className="bg-surface border rounded-lg p-5"><h2 className="font-semibold mb-3">ATS compatibility</h2><div className="grid md:grid-cols-3 gap-2 text-sm">{Object.entries(data?.adapterCapabilities ?? {}).map(([ats, status]) => <div key={ats} className="border rounded p-3"><strong className="capitalize">{ats}</strong><div className={status === "PRODUCTION_FILL_TESTED" || status === "PRODUCTION_INSPECTED" ? "text-verified" : "text-caution"}>{status}</div></div>)}</div></section>
    {output && <pre className="bg-sunken text-primary border border-hairline rounded-lg p-4 overflow-auto text-xs whitespace-pre-wrap">{output}</pre>}
  </div>;
}
