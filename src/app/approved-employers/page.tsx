...
export default function ApprovedEmployersPage() {
  ...
  return <div className="max-w-[1500px] mx-auto px-8 py-10 space-y-6">
    <header className="flex items-start justify-between gap-4">
      <div><h1 className="text-2xl font-semibold">497 Approved Engineering Employers</h1><p className="text-sm text-slate-600">Approved portals and live jobs are separate trust states. <strong>APPROVED_OFFICIAL_PORTAL</strong> allowlists an employer portal; <strong>LIVE_JOB_VERIFIED</strong> means one exact internship was confirmed open there.</p></div>
      <button onClick={runImport} disabled={importing} className="rounded-lg bg-brand text-white px-4 py-2.5 text-sm disabled:opacity-40">{importing ? "Importing…" : "Import CSV"}</button>
    </header>
    {status && <section className={`rounded-xl border p-5 ${data?.fileExists && status.ran ? "bg-emerald-50 border-emerald-200" : "bg-rose-50 border-rose-300"}`}>
      <h2 className="font-semibold">Import status</h2>
      {!data?.fileExists && <p className="font-medium text-rose-800 mt-2">Required CSV is missing. No demo or mock employers are substituted.</p>}
      ...
      {status.errors?.length > 0 && <ul className="mt-3 text-xs text-rose-800 list-disc pl-5">{status.errors.map((error) => <li key={error}>{error}</li>)}</ul>}
    </section>}
    <section className="bg-white rounded-xl border border-slate-200 p-4 grid md:grid-cols-3 gap-3">
    ...
    <div className="bg-white rounded-xl border border-slate-200 overflow-x-auto"><table className="w-full text-xs"><thead><tr className="text-left uppercase text-slate-500 border-b">{["Employer","Sector","Official career website","Career domain","EE/CPE fit","Portal status","Last curated","Search terms","Last checked","Live internships","Monitoring"].map((h)=><th key={h} className="p-3">{h}</th>)}</tr></thead><tbody>{data?.employers.map((e)=><tr key={e.id} className="border-b last:border-0 align-top"><td className="p-3 font-medium">{e.name}</td><td className="p-3">{e.csvSector ?? "—"}</td><td className="p-3">{e.careersUrl ? <a className="text-brand hover:underline" href={e.careersUrl} target="_blank" rel="noreferrer">Official careers ↗</a> : "Missing"}</td><td className="p-3">{e.csvCareerDomain ?? "—"}</td><td className="p-3">{e.csvEeCpeFit ?? "—"}</td><td className="p-3"><span className="font-medium text-emerald-700">{e.portalStatus}</span></td><td className="p-3">{e.csvVerifiedDate ? new Date(e.csvVerifiedDate).toLocaleDateString() : "—"}</td><td className="p-3 max-w-64">{e.csvRecommendedSearchTerms ?? "—"}</td><td className="p-3">{e.lastCheckedAt ? new Date(e.lastCheckedAt).toLocaleString() : "Never"}</td><td className="p-3 font-semibold">{e.currentlyVerifiedInternshipOpenings}</td><td className="p-3 capitalize">{e.monitoringStatus}</td></tr>)}</tbody></table>{data && data.employers.length === 0 && <p className="p-6 text-sm text-slate-500">No approved employers imported.</p>}</div>
...
function Metric({label,value}:{label:string;value:string|number}) { return <div><dt className="text-slate-500">{label}</dt><dd className="font-medium break-words">{value}</dd></div>; }
