  ...
  return (
    <div className="max-w-4xl mx-auto px-8 py-10 space-y-6">
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        <h2 className="font-medium text-slate-900">Application profile (used on generated resume headers)</h2>
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        <h2 className="font-medium text-slate-900">Application-form answers (optional)</h2>
      ...
      <section className="bg-white rounded-xl border border-slate-200 p-6 space-y-4">
        <div className="flex items-center justify-between">
          <h2 className="font-medium text-slate-900">Bullet library ({bullets.length})</h2>
          <button
        ...
        {genMessage && <p className="text-xs text-slate-600">{genMessage}</p>}
        {bullets.length === 0 ? (
          <p className="text-sm text-slate-500">No bullets yet. Approve resume facts on the Profile page, then click Regenerate.</p>
        ) : (
          <div className="space-y-4">
            {Object.entries(bulletsByCategory).map(([category, items]) => (
              <div key={category}>
                <h3 className="text-xs uppercase tracking-wide font-semibold text-slate-500 mb-2">{category}</h3>
                <div className="space-y-1.5">
                  {items.map((b) => (
                    <div key={b.id} className="text-sm text-slate-700 border border-slate-100 rounded-lg px-3 py-2">
                      {b.text}
                    </div>
                  ))}
                </div>
